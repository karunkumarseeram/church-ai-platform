...
export default function Dashboard() {
  ...
  const gradients = {
    members: "linear-gradient(135deg, #6A1B9A, #9C27B0)",
    events: "linear-gradient(135deg, #FF6B6B, #FFD93D)",
    donations: "linear-gradient(135deg, #4D96FF, #6BCB77)",
    prayers: "linear-gradient(135deg, #FF9800, #FFC107)",
    section: "linear-gradient(135deg, #E3F2FD, #E1BEE7)",
    upcoming: [
      "#6A1B9A, #9C27B0",
      "#8E24AA, #BA68C8",
      "#AB47BC, #CE93D8",
  ...
  const getGradientColor = (idx) =>
    gradients.upcoming[idx % gradients.upcoming.length].split(",").map(c => c.trim()).join(",");
  ...
  return (
    <div style={{...styles.container, background: "linear-gradient(135deg, #E6E6FA, #ADD8E6)"}}>
      <AdminDashboardHeader userRole={userRole} />

      {/* Stats */}
      <div style={styles.grid}>
        {isAdmin && (
          <div style={{...styles.card, background: gradients.members}} onClick={()=>navigate("/members")}>
            <h3>👥 Members</h3>
            <p style={styles.statNumber}>{stats.members}</p>
          </div>
        )}
        <div style={{...styles.card, background: gradients.events}} onClick={()=>navigate("/events")}>
          <h3>📅 Events</h3>
          <p style={styles.statNumber}>{stats.events + defaultEvents.length}</p>
        </div>
        <div style={{...styles.card, background: gradients.donations}} onClick={()=>navigate("/donations")}>
          <h3>💳 Donations</h3>
          <p style={styles.statNumber}>₹{stats.donations}</p>
        </div>
        <div style={{...styles.card, background: gradients.prayers}} onClick={()=>navigate("/prayers")}>
          <h3>🙏 Prayer Requests</h3>
          <p style={styles.statNumber}>{stats.prayers || 0}</p>
        </div>
      </div>

      {/* Upcoming Events */}
      <div style={{...styles.section, background: gradients.section}}>
...
const styles = {
  container: {padding:30, minHeight:"100vh"},
